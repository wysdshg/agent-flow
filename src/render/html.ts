...
function moduleFills(n){var base=COLORS[n.state]||"#959da5";
...
function nodeSvg(n){var fills=moduleFills(n);var fill=fills[0];
var tc=(fills.length>1)?((fills.filter(function(c){return darkText(c);}).length*2>=fills.length)?"#ffffff":"#1f2430"):(darkText(fill)?"#1f2430":"#ffffff");
var stroke="rgba(20,30,50,0.28)";var w=n.w,h=n.h;
...
else if(PORTED[n.type]){var fattr=fill;
if(fills.length>1){var gid="ag-"+esc(cur)+"-"+esc(n.id);var stops="";var seg=100/fills.length;
fills.forEach(function(c,i){stops+='<stop offset="'+(i*seg)+'%" stop-color="'+c+'"/><stop offset="'+((i+1)*seg)+'%" stop-color="'+c+'"/>';});
g+='<defs><linearGradient id="'+gid+'" x1="0" y1="0" x2="1" y2="0">'+stops+"</linearGradient></defs>";fattr="url(#"+gid+")";}
g+='<rect class="body" width="'+w+'" height="'+h+'" rx="10" fill="'+fattr+'" stroke="'+stroke+'" stroke-width="2.2"/>';return finishNode(g,n,tc,true,false);}
